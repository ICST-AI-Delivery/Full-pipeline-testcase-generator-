"""
Simple CLIP Pipeline - Process any CSV with image paths using existing code
...
def auto_generate_csv(feature_name):
    ...
    # Define possible directory paths to search (prioritize root location)
    base_paths = [
        f"SRS FPI Export/SRS_Instrument Cluster/{feature_name}",
        f"SRS FPI Export/{feature_name}",
        f"data/raw/SRS_FPI_Export/SRS_Instrument Cluster/{feature_name}",
        f"data/raw/SRS_FPI_Export/{feature_name}",
    ...
    csv_output_path = Path("data/raw") / f"{feature_name}.csv"
...
def process_csv_with_clip(input_csv,
                          ...
                          model_weights=None,
                          temperature=0.07,
                          merged_table_path="data/processed/merged_clip_predictions.csv",
    ...
    if output_csv is None:
        ...
        if "ground_truth_template_with_clip_predictions" in input_csv:
            ...
            # Check if we're already in a data directory
            if input_dir.endswith("data/raw") or input_dir.endswith("data/processed"):
                # Use the same directory (don't create nested data directories)
                out_dir = input_dir
            else:
                # Create data/processed subdirectory for output
...
def parse_args():
    ...
    parser.add_argument("--merged-table", type=str, default="data/processed/merged_clip_predictions.csv",
...
def main():
    ...
    if not input_value:
        print("Usage:")
        print("  Feature mode: python simple_clip_pipeline.py --feature-mode VEH-F165_Manettino")
        print("  CSV mode:     python simple_clip_pipeline.py path/to/input.csv [path/to/output.csv]")
        print("\nAvailable options:")
        ...
        # Show some example features if in feature mode
        if feature_mode:
            print("\nExample features:")
            print("  VEH-F165_Manettino")
            print("  VEH-F844_Matrix_State_Event")
            print("  VEH-F001_Immobilizer")
        
        # Fall back to interactive mode (process known CSVs if they exist)
        csv_files = [
            "data/raw/ground_truth_template_with_clip_predictions.csv",
            "data/raw/additional_images_with_old_clip_descriptions.csv"
        ]
        ...
        print("\nProcessing available CSV files with default settings...")
        default_models = [m.strip() for m in args.models.split(",") if m.strip()]
    ...
    if feature_mode:
        ...
        base_csv = f"data/raw/{feature_name}.csv"
        ...
        output_csv = f"data/processed/{feature_name}_with_clip_predictions.csv"
