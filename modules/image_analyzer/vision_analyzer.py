...
def load_prompt(category):
    ...
    if not prompt_path.exists():
        print(f"Warning: Prompt file {prompt_file} not found, using general analysis")
        return "Please analyze this technical image in detail, focusing on all visible elements, text, symbols, and their relationships."
...
def save_analysis(analysis_text, feature_name, image_name, category):
    ...
    output_dir = Path("analysis_results") / feature_name
...
def process_csv(csv_file, feature_name, api_key):
    ...
    print(f"Results saved to: analysis_results/{feature_name}/")
...
def main():
    ...
    clip_csv = f"data_files/{feature_name}_with_clip_predictions.csv"
    if not Path(clip_csv).exists():
        ...
        print(f"python scripts/simple_clip_pipeline.py --feature-mode {feature_name}")
        sys.exit(1)
    ...
    if success:
        ...
        print(f"   • Analysis results: analysis_results/{feature_name}/")
    else:
