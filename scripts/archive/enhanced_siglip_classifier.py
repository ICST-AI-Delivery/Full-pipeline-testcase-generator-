"""
Enhanced SigLIP-based Image Classifier for Automotive Technical Diagrams
Features improved automotive-specific prompts for better classification accuracy
"""
...
class EnhancedSigLIPImageClassifier:
    def __init__(self, model_name="google/siglip-base-patch16-224", device="cpu"):
        """
        Initialize SigLIP model for image classification with enhanced automotive prompts
        """
        self.device = device
        print(f"Loading Enhanced SigLIP model: {model_name}")
        ...
        print(f"Initialized Enhanced SigLIP classifier with {len(self.categories)} categories")
        print("Enhanced with automotive-specific terminology for improved accuracy")
    
    def classify_image(self, image_path):
        """
        Classify a single image using enhanced SigLIP
        """
        try:
            # Load image
            ...
            # Create description
            description = f"Enhanced SigLIP classification: {self.text_descriptions[best_idx][:100]}... (score: {confidence_score:.3f})"
...
def update_csv_with_enhanced_siglip(csv_path, output_path=None):
    """
    Update existing CSV with Enhanced SigLIP classifications
    """
    if output_path is None:
        output_path = csv_path.replace('.csv', '_enhanced_siglip.csv')
    
    # Initialize classifier
    classifier = EnhancedSigLIPImageClassifier(device="cpu")
    ...
    with open(csv_path, 'r', encoding='utf-8') as file:
        ...
        new_header = header + ['Enhanced SigLIP Category', 'Enhanced SigLIP Confidence', 'Enhanced SigLIP Description']
        ...
        for row_idx, row in enumerate(reader, 1):
            ...
            # Run Enhanced SigLIP classification
            category, confidence, description = classifier.classify_image(image_path)
            
            # Add Enhanced SigLIP results to row
    ...
    print(f"Enhanced SigLIP results saved to: {output_path}")

if __name__ == "__main__":
    # Test with ground truth template
    csv_path = "data_files/ground_truth_template.csv"
    if os.path.exists(csv_path):
        update_csv_with_enhanced_siglip(csv_path)
    else:
